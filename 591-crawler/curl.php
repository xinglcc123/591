...
function multi_thread_curl($request_array, $number_threads)
{
	$results = [];

	$chunks = array_chunk($request_array, $number_threads);
	foreach ($chunks as $chunk) {
		// Do curl multi requests多個請求
		$chArr = [];
		foreach ($chunk as $i => $request) {
			...
			curl_setopt($chArr[$i], CURLOPT_USERAGENT, 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/92.0.4515.159 Safari/537.36');
			curl_setopt($chArr[$i], CURLOPT_HEADER, false);
			curl_setopt($chArr[$i], CURLOPT_RETURNTRANSFER, true);
			curl_setopt($chArr[$i], CURLOPT_HTTPHEADER, $headers);
			# curl_setopt($chArr[$i], CURLOPT_COOKIEFILE, "cookie.txt");
		}
		...
		foreach ($chunk as $i => $request) {
			$response = curl_multi_getcontent($chArr[$i]);
			curl_multi_remove_handle($mh, $chArr[$i]);
			curl_close($chArr[$i]);

			$results[] = ['request' => $request, 'response' => $response];
		}
...
curl_setopt($ch, CURLOPT_URL, "https://rent.591.com.tw/");
$page = curl_exec($ch);
curl_close($ch);
...
$results = multi_thread_curl($requests, $curlPoolSize);
...
foreach ($rows as $row) {
	...
	$rent_price  = max(array_map(function ($price) {
		return str_replace(',', '', $price);
	}, array_filter(explode('~', $row['price']))));
